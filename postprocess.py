...
agg = (df.groupby(grp)
       .agg(mean_runtime=("runtime","mean"),
            ...
            succ_rate=("failed_iterations",lambda x:(x==0).mean()))
       .reset_index())
...
sns.boxplot(data=df[df.satProb>0], x="k", y="sat_ratio_ops", hue="algo")
plt.ylabel("SAT % z ops runtime"); plt.savefig(RES/"fig_sat_share.png", dpi=300)

# ---------------- significance ------------
def sig_test(sub):
    pp  = sub[(sub.algo=="PP") & (sub.satProb==0)]["final_soc"]
    sat = sub[(sub.algo=="SAT") & (sub.satProb==100)]["final_soc"]
    return wilcoxon(pp, sat).pvalue if len(pp)==len(sat)==10 else np.nan
sig = df.groupby(["map","k"]).apply(sig_test).rename("p").reset_index()
